...
from paddle.nn.initializer import XavierNormal,KaimingNormal ,Normal,Constant


import paddle.nn.functional as F

from paddle.optimizer.lr import LRScheduler,LambdaDecay
import paddle
import functools
...
def get_scheduler(optimizer, args):
    ...
    if args.lr_policy == 'linear':
        def lambda_rule(epoch):
            ...
            return lr_l
        # scheduler = LRScheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)
        scheduler = LambdaDecay(learning_rate=0.001,lr_lambda=lambda_rule)
    elif args.lr_policy == 'step':
        step_size = args.max_epochs//3
        # args.lr_decay_iters
        scheduler = LRScheduler.StepLR(optimizer, step_size=step_size, gamma=0.1)
    else:
    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            if init_type == 'normal':
                ...
                Normal(m.weight, 0.0, init_gain)
            elif init_type == 'xavier':
                XavierNormal(m.weight.data, gain=init_gain)
            elif init_type == 'kaiming':
                KaimingNormal(m.weight.data, a=0, mode='fan_in')
            elif init_type == 'orthogonal':
                nn.initializer.orthogonal_(m.weight.data, gain=init_gain)
            else:
                raise NotImplementedError('initialization method [%s] is not implemented' % init_type)
            if hasattr(m, 'bias') and m.bias is not None:
                Constant(m.bias)
        elif classname.find('BatchNorm2d') != -1:  # BatchNorm Layer's weight is not a matrix; only normal distribution applies.
            nn.initializer.normal_(m.weight.data, 1.0, init_gain)
            nn.initializer.constant_(m.bias.data, 0.0)
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if len(gpu_ids) > 0:
        ...
        net.to(gpu_ids[0])
        if len(gpu_ids) > 1:
            net = paddle.nn.DataParallel(net, gpu_ids)  # multi-GPUs
...
class ResNet(nn.Layer):
    def __init__(self, input_nc, output_nc,
        ...
        if backbone == 'resnet18':
            self.resnet = resnet18(pretrained=True, strides=strides)
        elif backbone == 'resnet34':
            self.resnet = models.resnet34(pretrained=True, strides=strides)
        else:
...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
        ...
        if with_pos == 'learned':
            # self.pos_embedding = nn.Embedding(paddle.randn(1, self.token_len*2, 32))
            # self.pos_embedding =self.create_parameter(paddle.randn(1, self.token_len*2, 32))
            self.pos_embedding =self.create_parameter(shape=(1, self.token_len*2, 32))

        decoder_pos_size = 256//4
        self.with_decoder_pos = with_decoder_pos
        if self.with_decoder_pos == 'learned':
            self.pos_embedding_decoder =nn.ParameterList(paddle.randn(1, 32,
                                                                 decoder_pos_size,
                                                                 decoder_pos_size))
    ...
    def _forward_semantic_tokens(self, x):
        ...
        spatial_attention = paddle.nn.functional.softmax(spatial_attention, axis=-1)
        x = x.reshape([b, c, -1])
        tokens = paddle.einsum('bln,bcn->blc', spatial_attention, x)
    ...
    def _forward_reshape_tokens(self, x):
        ...
        if self.pool_mode == 'max':
            ...
            x = x
        b, c, h, w = x.shape
        tokens = x.transpose((0,2,3,1)).reshape((b, -1, c))
        return tokens
    ...
    def forward(self, x1, x2):
        ...
        if self.token_trans:
            self.tokens_ = paddle.concat([token1, token2], axis=1)
