...
from misc.std_mean import std_mean
...
from paddle.nn import MaxPool1D,AvgPool1D
...
def norm_tensor(tensor,min_=None,max_=None, mode='minmax'):
    ...
    if mode == 'minmax':
        ...
        mean, std = std_mean(tensor, [-1], keepdim=True)
        ...
        tai = 1
        tensor = F.softmax(tensor/tai, dim=-1, )
        min_, _ = tensor.min(-1, keepdim=True)
        max_, _ = tensor.max(-1, keepdim=True)
        tensor = (tensor - min_) / (max_ - min_ + 0.00000000001)
    else:
...
def visulize_features(features, normalize=False):
    ...
    from datasets import make_grid
...
def np_to_tensor(image):
    ...
    if isinstance(image, paddle.to_tensor):
        return image
...
def simplex(t: Tensor, axis=1) -> bool:
    _sum = t.sum(axis).type(paddle.float32)
    _ones = paddle.ones_like(_sum, dtype=paddle.float32)
    return paddle.allclose(_sum, _ones)


# Assert utils
def uniq(a: Tensor) -> Set:
    return set(paddle.unique(a.cpu()).numpy())

def sset(a: Tensor, sub: Iterable) -> bool:
    return uniq(a).issubset(sub)

def eq(a: Tensor, b) -> bool:
    return paddle.equal(a, b).all()
...
def class2one_hot(seg: Tensor, C: int) -> Tensor:
    ...
    res = paddle.stack([seg == c for c in range(C)], dim=1).type(paddle.int32)
...
def cross_entropy(input, target, weight=None, reduction='mean',ignore_index=255):
    ...
    target = target.long()
    if target.dim() == 4:
        target = paddle.squeeze(target, dim=1)
...
def balanced_cross_entropy(input, target, weight=None,ignore_index=255):
    ...
    pos = (target==1).float()
    neg = (target==0).float()
    pos_num = paddle.sum(pos) + 0.0000001
    neg_num = paddle.sum(neg) + 0.0000001
    # print(pos_num)
    # print(neg_num)
...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        ...
        scheduler = LRScheduler.StepLR(optimizer, step_size=opt.lr_decay_iters, gamma=0.1)
    else:
...
def mul_cls_acc(preds, targets, topk=(1,)):
    ...
    with paddle.no_grad():
        ...
        correct = paddle.zeros([bs, maxk]).long()  # 记录预测正确label数量
        if preds.device != paddle.set_device(type='cpu'):
...
class PolyOptimizer(paddle.optimizer.SGD):
...
class PolyAdamOptimizer(paddle.optimizer.Adam):
...
class SGDROptimizer(paddle.optimizer.SGD):
...
def gap2d(x, keepdims=False):
    out = paddle.mean(x.view(x.size(0), x.size(1), -1), -1)
    if keepdims:
...
def decode_seg(label_mask, toTensor=False):
    ...
    if not isinstance(label_mask, np.ndarray):
        if isinstance(label_mask, paddle.to_tensor):  # get the data from a variable
    ...
    rgb[:, :, 2] = b / 6
    if toTensor:
        rgb = paddle.to_tensor(rgb.transpose([2,0,1])).unsqueeze(0)
...
def tensor2im(input_image, imtype=np.uint8, normalize=True):
    ...
    if not isinstance(input_image, np.ndarray):
        if isinstance(input_image, paddle.to_tensor):  # get the data from a variable
