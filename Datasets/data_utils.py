...
from paddle.vision.transforms import Normalize,Resize,functional,RandomResizedCrop,ColorJitter,resize,ToTensor
from paddle import to_tensor as to
from Datasets.pil_image import to_pil_image as to_pil_image
# import torchvision.transforms.functional as TF
...
#     labels = [paddle.to_tensor(np.array(img, np.uint8)).unsqueeze(dim=0)
#               for img in labels]
#
#     # imgs = [TF.normalize(img, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
#     imgs = [Normalize(img, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
#             for img in imgs]
#
#
#     return imgs, labels
    ...
    def transform(self, imgs, labels, to_tensor=True):
        ...
        if not self.img_size_dynamic:
            ...
            if imgs[0].size != (self.img_size, self.img_size):
                imgs = [resize(img, self.img_size)
                        for img in imgs]

        else:
            self.img_size = imgs[0].size[0]

        labels = [to_pil_image(img) for img in labels]
        if len(labels) != 0:
            if labels[0].size != (self.img_size, self.img_size):
                labels = [resize(img, self.img_size)
                        for img in labels]

        random_base = 0.5
        if self.with_random_hflip and random.random() > 0.5:
            imgs = [functional.hflip(img) for img in imgs]
            labels = [functional.hflip(img) for img in labels]

        if self.with_random_vflip and random.random() > 0.5:
            imgs = [functional.vflip(img) for img in imgs]
            labels = [functional.vflip(img) for img in labels]

        if self.with_random_rot and random.random() > random_base:
            angles = [90, 180, 270]
            index = random.randint(0, 2)
            angle = angles[index]
            imgs = [functional.rotate(img, angle) for img in imgs]
            labels = [functional.rotate(img, angle) for img in labels]

        if self.with_random_crop and random.random() > 0:
            i, j, h, w = RandomResizedCrop(size=self.img_size). \
                get_params(img=imgs[0], scale=(0.8, 1.2), ratio=(1, 1))

            imgs = [functional.resized_crop(img, i, j, h, w,
                                    size=(self.img_size, self.img_size),
                                    interpolation=Image.CUBIC)
                    for img in imgs]

            labels = [functional.resized_crop(img, i, j, h, w,
                                      size=(self.img_size, self.img_size),
        ...
        if to_tensor:
            ...
            # labels = [paddle.to_tensor(np.array(img, np.uint8)).unsqueeze(axis=0)
            labels = [to(np.asarray(img).astype(np.float32)).unsqueeze(axis=0)
            # labels = [to(np.asarray(img).astype(np.int64)).unsqueeze(axis=0)
                      for img in labels]
