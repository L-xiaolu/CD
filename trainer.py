...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if args.optimizer == "sgd":
            self.optimizer_G = optim.SGD(learning_rate=lr_scheduler,
            ...
            self.optimizer_G = optim.AdamW(learning_rate=lr_scheduler,
                                           parameters=self.net_G.parameters(),
        ...
        if args.loss == 'ce':
            ...
            weights = 1-paddle.to_tensor(alpha).cuda()
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.'):
        print("\n")
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            self.logger.write('loading last checkpoint...\n')
            # load the entire checkpoint
            checkpoint = paddle.load(os.path.join(self.checkpoint_dir, ckpt_name + 'pdparams'))
            # update net_G states
            # self.net_G.load_state_dict(checkpoint['model_G_state_dict'])
            self.net_G.set_state_dict(checkpoint)
            opt_state_dict = paddle.load(os.path.join(self.checkpoint_dir, ckpt_name + 'pdparams'))
            self.optimizer_G.set_state_dict(opt_state_dict)
            ...
            self.net_G.load_state_dict(paddle.load(self.args.pretrain), strict=False)
            self.net_G.eval()
        else:
    ...
    def _visualize_pred(self):
        pred = paddle.argmax(self.G_final_pred, axis=1, keepdim=True)
        pred_vis = pred * 255
        return pred_vis
    ...
    def _save_checkpoint(self, ckpt_name):
        ...
        }, os.path.join(self.checkpoint_dir, ckpt_name + 'pdparams'))

        # paddle.save({
        #     self.net_G.state_dict()
        # }, os.path.join(self.checkpoint_dir, ckpt_name + 'pdparams'))

        # paddle.save({self.optimizer_G.state_dict()},os.path.join(self.checkpoint_dir, ckpt_name + 'pdopt'))
    ...
    def _collect_running_batch_states(self):
        ...
        if np.mod(self.batch_id, 500) == 1:
            ...
            vis_gt = utils.make_numpy_grid(self.batch['L'].unsqueeze(1))
    ...
    def _update_checkpoints(self):
        ...
        # save current model
        self._save_checkpoint(ckpt_name='last_ckpt.')
        ...
        if self.epoch_mf1 > self.mF1:
            # self.best_val_acc = self.epoch_acc
            self.best_val_acc = self.epoch_acc
            self.mF1 = self.epoch_mf1
    ...
    def _forward_pass(self, batch):
        ...
        if self.multi_scale_infer == "True":
            self.G_final_pred = paddle.zeros(self.G_pred[-1].size())
    ...
    def _backward_G(self):
        gt = self.batch['L'].astype('float32')
